...
class DialProgramsTasksGenerator(AbstractTasksGenerator):
    ...
    def _generate_strokes_strings_for_stimuli(
        ...
        if context == CONTEXT_LARGE_ABSTRACTIONS:
            possible_rows = [1]
            possible_tiers = [1]
            possible_base_end_filials = [True]
            possible_add_side_antenna = False
            possible_dial_sizes = [SMALL]
            possible_shape_specifications = [
                [c_string, r_string],
            ]
            generation_probability = 1.0
            ...
            possible_rows = [1, 2]
            possible_tiers = [1, 2]
            possible_base_end_filials = [True, False]
            possible_add_side_antenna = True
            possible_dial_sizes = [SMALL, LARGE]
            possible_shape_specifications = [
                None,
                [c_string, r_string],
                [c_string, c_string],
        ...
        for total_dials in total_dials_range:
            ...
            total_columns_range = [max_dials + 1]

            # Varying bases for the single small dials.
            for base_columns in total_columns_range:
                for base_heights in [1, max_dials]:
                    for rows in possible_rows:
                        for tiers in possible_tiers:
                            for base_end_filials in possible_base_end_filials:
                                ...
                                add_side_antenna = possible_add_side_antenna and (
                                ...
                                # Small and large dials with the lever sticking out.
                                for dial_size in possible_dial_sizes:
                                    for dial_angle in [STR_VERTICAL]:
                                        for (
                                            shape_specification
                                        ) in possible_shape_specifications:
                                            if (
                                                dial_size == LARGE
                                                and dial_angle == STR_VERTICAL
                                            ):
